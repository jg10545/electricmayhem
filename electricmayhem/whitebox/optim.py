...
def _get_optimizer_and_scheduler(optimizer, params, lr, decay="none", steps=None):
    ...
    if opt == "nifgsm":
        logging.warning(
            "THIS NIFGSM IMPLEMENTATION IS AN EARLY PROTOTYPE YOU PROBABLY SHOULDNT ACTUALLY USE FOR ANYTHING"
        )
    ...
    if decay == "none":
        ...
        gamma = (1e-3) ** (1.0 / max(steps, 1))  # max() to prevent error when steps=0
        scheduler = torch.optim.lr_scheduler.ExponentialLR(opt, gamma)
    elif decay == "plateau":
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            opt, mode="min", factor=0.1, patience=100
        )
