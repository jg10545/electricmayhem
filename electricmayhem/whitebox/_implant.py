...
def _unpack_rectangle_dataset(df):
    ...
    if "split" not in df.columns:
        ...
        img_keys_train, images_train, boxes_train = _unpack_rectangle_frame(
            df[df["split"] == "train"]
        )
        img_keys_eval, images_eval, boxes_eval = _unpack_rectangle_frame(
            df[df["split"] == "eval"]
        )
        return (
            df,
...
class RectanglePatchImplanter(PipelineBase):
    ...
    def __init__(
        ...
        self._sample_offsets = (
            True  # whether self.sample() should sample offsets relative to the box
        )
    ...
    def _get_mask_summary(self, mask):
        ...
        if mask is None:
            ...
            assert (
                torch.max(mask) <= 1 and torch.min(mask) >= 0
            ...
            for k in mask:
                if isinstance(mask[k], torch.Tensor):
                    assert (
                        torch.max(mask[k]) <= 1 and torch.min(mask[k]) >= 0
                    ), "mask should be between 0 and 1"
                else:
                    assert (
                        mask[k] >= 0 and mask[k] <= 1
                    ), "mask should be between 0 and 1"
                return "dict"
        else:
        ...
        for k in self.patch_keys:
            if self._sample_scale:
                if f"scale_{k}" not in kwargs:
                    sampdict[f"scale_{k}"] = torch.FloatTensor(n).uniform_(
                        p["scale"][0], p["scale"][1]
                    )
            ...
            if f"box_{k}" not in kwargs:
                i = torch.tensor(
                    [
                        torch.randint(low=0, high=len(boxes[j][k]), size=[])
                        for j in sampdict["image"]
                    ]
                )
                sampdict[f"box_{k}"] = i
            if self._sample_offsets:
                if f"offset_frac_x_{k}" not in kwargs:
                    if p["offset_frac_x"] is None:
                        ...
                        sampdict[f"offset_frac_x_{k}"] = torch.tensor(
                            n * [p["offset_frac_x"]]
                        )
                if f"offset_frac_y_{k}" not in kwargs:
                    if p["offset_frac_y"] is None:
                        ...
                        sampdict[f"offset_frac_y_{k}"] = torch.tensor(
                            n * [p["offset_frac_y"]]
                        )
        ...
        if self.mask is None:
            ...
            if isinstance(self.mask, torch.Tensor):
                with torch.no_grad():
                    ...
                    mask = kornia.geometry.resize(
                        mask, (patch.shape[1], patch.shape[2])
                    )
            # scalar mask case
            else:
                mask = self.mask
    ...
    def _implant_patch(
        self, image, patch, offset_x, offset_y, mask, scale_brightness=False
    ):
        implanted = []
        for i in range(len(image)):
            ...
            with torch.no_grad():
                cutout = imp.clone().detach()[
                    :, offset_y[i] : offset_y[i] + pH, offset_x[i] : offset_x[i] + pW
                ]
            if scale_brightness:
                with torch.no_grad():
    ...
    def forward(self, patches, control=False, evaluate=False, params={}, **kwargs):
            ...
            patchlist = {
                k: [
                    kornia.geometry.rescale(
                        patches[k][i].unsqueeze(0),
                        (s[f"scale_{k}"][i], s[f"scale_{k}"][i]),
                    ).squeeze(0)
                    for i in range(patches[k].shape[0])
                ]
                for k in patches
            }
        else:
            patchlist = {
                k: [patches[k][i] for i in range(patches[k].shape[0])] for k in patches
            }
        ...
        for k in patches:
            offset_x, offset_y = get_pixel_offsets_from_fractional_offsets(
                s, boxes, patchlist[k], k
            )
            images = self._implant_patch(
                images,
                patchlist[k],
                offset_x,
                offset_y,
                self.mask[k],
                self.params["scale_brightness"],
            )
                    ...
                    for k in self.patch_keys:
                        for j in range(len(boxes[i][k])):
                            ...
                            rect = matplotlib.patches.Rectangle(
                                xw, width, height, linewidth=2, fill=False, color="r"
                            )
                            ax.add_artist(rect)
    ...
    def get_last_sample_as_dict(self):
        ...
        for k in self.lastsample:
            if k == "image":
                outdict["image"] = [
                    imgkeys[i] for i in self.lastsample["image"].cpu().detach().numpy()
                ]
            else:
                outdict[k] = [
                    float(i) for i in self.lastsample[k].cpu().detach().numpy()
                ]
        return outdict
...
class FixedRatioRectanglePatchImplanter(RectanglePatchImplanter):
    ...
    def _add_patch_scales_to_sampdict(self, s, patches, evaluate=False):
        ...
        for k in self.patch_keys:
            ...
            s[f"scale_{k}"] = torch.Tensor(
                patch_scales
            )  # does this need to be a tensor or could I
            # leave as a list?
    ...
    def forward(self, patches, control=False, evaluate=False, params={}, **kwargs):
        ...
        patchlist = {
            k: [
                kornia.geometry.rescale(
                    patches[k][i].unsqueeze(0), (s[f"scale_{k}"][i], s[f"scale_{k}"][i])
                ).squeeze(0)
                for i in range(patches[k].shape[0])
            ]
            for k in patches
            ...
            for i in range(bs):
                ...
                patchbatch.append(
                    kornia.geometry.resize(
                        patches[k][i].unsqueeze(0), (box_h, box_w)
                    ).squeeze(0)
                )
