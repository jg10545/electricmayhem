...
class NPSCalculator(torch.nn.Module):
    ...
    def __init__(self, patch_size, printability_file=None):
        ...
        self.printability_array = torch.nn.Parameter(
    ...
    def forward(self, adv_patch):
        ...
        for i in range(adv_patch.shape[0]):
            color_dist = (
                adv_patch[i] - self.printability_array + 0.000001
            )  # (num_prints, C, H, W)
...
def saliency_loss(img):
    ...
    return torch.sqrt(
        torch.var(rg, dim=(1, 2)) + torch.var(yb, dim=(1, 2))
    ) + 0.3 * torch.sqrt(
        torch.mean(rg, dim=(1, 2)) ** 2 + torch.sqrt(torch.mean(yb, dim=(1, 2)) ** 2)
    )
