...
def eval_result_permutation_importance(df, metric, n_estimators=100, test_size=0.3, n_repeats=25,
                                       max_features_to_plot=25, n_jobs=-1):
    """
    ...
    metric_names = [c for c in df.columns if c.endswith("_patch")|c.endswith("_delta")|c.endswith("_control")]
    ...
    if len(categoricals) > 0:
        for c in categoricals:
            newdf = pd.concat([newdf, pd.get_dummies(df[c], prefix=c, dtype='int')], axis=1)
    ...
    perm_result = sklearn.inspection.permutation_importance(forest, X_test, Y_test,
                                                        n_repeats=n_repeats, n_jobs=n_jobs)

    # now plot it
    sorted_importances_idx = perm_result.importances_mean.argsort()
    importances = pd.DataFrame(perm_result.importances[sorted_importances_idx].T,
