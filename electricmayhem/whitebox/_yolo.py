...
def convert_v4_to_v5_format(output, H, W):
    ...
    return [
        torch.concatenate([box_dims[:, :, 0, :], max_score.unsqueeze(-1), confs], -1)
    ]
...
def convert_ultralytics_to_v5_format(detections):
    ...
    """
    maxdetect = torch.max(detections[:, 4:, :], 1)[0].unsqueeze(
        1
    )  # [batch_size, 1, num_boxes]
    newdets = torch.concat(
        [
            detections[:, :4, :],  # [batch_size, 4, num_boxes]
            maxdetect,  # [batch_size, 1, num_boxes]
            detections[:, 4:, :],
        ],  # [batch_size, num_classes, num_boxes]
        1,
    )  # [batch_size, 5+num_classes, num_boxes]
    ...
    for i in ordering:
        ...
        if plotbox:
            if scores[i] > 0.5:
                ...
                color = "red"
                lw = 1
            det_rect = matplotlib.patches.Rectangle(
                (boxes[i, 0], boxes[i, 1]),
                boxes[i, 2] - boxes[i, 0],
                boxes[i, 3] - boxes[i, 1],
                facecolor="none",
...
def _plot_detection_pair_to_array(
    im1, dets1, im2, dets2, k, classnames=None, thresh=0.1, iouthresh=0.5
):
    """
    Wrapper for plot_detections that plots a pair of images, converts to a
    numpy array, and concatenates them
    """
    fig1 = plot_detections(
        im1, dets1, k, classnames=classnames, thresh=thresh, iouthresh=iouthresh
    )
    fig2 = plot_detections(
        im2, dets2, k, classnames=classnames, thresh=thresh, iouthresh=iouthresh
    )
...
class YOLOWrapper(ModelWrapper):
    ...
    def forward(self, x, control=False, evaluate=False, **kwargs):
        ...
        if wraptype == "list":
            assert (
                False
            ), "wrapping models as a list no longer supported; please use a dictionary"
        # dictionary of models
        elif wraptype == "dict":
            # every model has same YOLO version case
            if "yolo_version" in self.params:
                outputs = {
                    o: self._convert_outputs(
                        outputs[o], self.params["yolo_version"], H, W
                    )
                    for o in outputs
                }
            # different YOLO verions for different models case
                ...
                outputs = {
                    o: self._convert_outputs(outputs[o], self.params[o], H, W)
                    for o in outputs
                }
        # single model
        else:
    ...
    def _vizualize_and_log_one_model_detection(
        self,
        x,
        x_control,
        detects,
        ...
        writer.add_images(
            logname, fig_arrays[:, :, :, :3], global_step=step, dataformats="NHWC"
        )
    ...
    def log_vizualizations(self, x, x_control, writer, step, logging_to_mlflow=False):
        ...
        if self._logviz:
            ...
            if self.eval_wraptype == "model":
                self._vizualize_and_log_one_model_detection(
                    x,
                    x_control,
                    detects[0],
                    detects_control[0],
