...
def perlin(H, W, period_y, period_x, octave, freq_sine, lacunarity=2, phase=0):
    ...
    for x in range(W):
        for y in range(H):
            noise[0, y, x] = pnoise2(
                x / period_x, y / period_y, octaves=octave, lacunarity=lacunarity
            )
...
class BayesianPerlinNoisePatchTrainer(BlackBoxPatchTrainer):
    ...
    def __init__(
        ...
        if load_from_json_file is not None:
            ...
            self.client.create_experiment(
                ...
                objectives={
                    "transform_robustness": ObjectiveProperties(minimize=False)
                },
            )
    ...
    def _generate_perturbation(self, **kwargs):
        ...
        perturbation[
            :, b["top"] : b["top"] + b["height"], b["left"] : b["left"] + b["width"]
        ] += noise
    ...
    def _run_trial(self, p):
        ...
        self.writer.add_scalar(
            "transform_robustness", tr_dict["tr"], global_step=self.query_counter
        )
        self.writer.add_scalar(
            "crash_frac", tr_dict["crash_frac"], global_step=self.query_counter
        )
    ...
    def _run_one_epoch(self, lrs=None, budget=None):
        ...
        self.client.complete_trial(
            trial_index=trial_index, raw_data=self._run_trial(parameters)
        )
    ...
    def crossvalidation_plot(self):
        ...
        ax.utils.notebook.plotting.render(
            ax.plot.diagnostic.interact_cross_validation(cv_results)
        )
