...
def augment_image(
    ...
    if blur > 0:
        image = kornia.filters.gaussian_blur2d(
            image, (2 * int(blur) + 1, 2 * int(blur) + 1), (blur, blur)
        )
    if rotate != 0:
        image = kornia.geometry.transform.rotate(
            image,
            torch.Tensor([rotate]),
            center=None,
            mode="bilinear",
...
def compose(img, mask, pert, angle=0, translate_x=0, translate_y=0):
    ...
    if (translate_x != 0) | (translate_y != 0):
        ...
        mask = kornia.geometry.transform.translate(
            mask.unsqueeze(0), translate
        ).squeeze()
        pert = kornia.geometry.transform.translate(
            pert.unsqueeze(0), translate
        ).squeeze()
