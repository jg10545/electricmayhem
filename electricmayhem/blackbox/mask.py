...
def generate_rectangular_frame_mask(
    W, H, xmin, ymin, xmax, ymax, frame_width=30, num_channels=3, return_torch=False
):
    """
    ...
    initial_mask[
        ymin - frame_width : ymax + frame_width,
        : xmin - frame_width : xmax + frame_width,
    ] += 1
    ...
    if return_torch:
        return torch.Tensor(initial_mask).permute(2, 0, 1), torch.Tensor(
            final_mask
        ).permute(2, 0, 1)
