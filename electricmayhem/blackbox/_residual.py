...
class ResidualBlackBoxPatchTrainer(BlackBoxPatchTrainer):
    ...
    def _estimate_gradient(self):
        ...
        if not hasattr(self, "_residual_tr"):
            self._residual_tr = estimate_transform_robustness(
                self.detect_func,
                augments,
                self.img,
                mask=mask,
        ...
        u = (
            torch.randn(self.perturbation.shape).type(torch.FloatTensor)
            * mask_resized[:C, :, :]
        )
        u = u / torch.norm(u)
        ...
        shifted_tr = estimate_transform_robustness(
            self.detect_func,
            augments,
            self.img,
            mask=mask,
        ...
        self.writer.add_scalar("shifted_tr", shifted_tr, global_step=self.query_counter)
