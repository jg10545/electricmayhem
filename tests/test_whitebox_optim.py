...
from electricmayhem.whitebox.optim import BIM, MIFGSM, NIFGSM
from electricmayhem.whitebox.optim import _get_optimizer_and_scheduler
...
def test_nifgsm():
    ...
    # and there should be a momentum buffer saved in the optimizer
    assert 'momentum_buffer' in list(opt.state.values())[0]
    assert 'x_adv' in list(opt.state.values())[0]
