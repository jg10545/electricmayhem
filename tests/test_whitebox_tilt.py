...
from electricmayhem.whitebox._tilt import sample_perspective_transforms, PerspectiveTilter
...
def test_perspectivetilter():
    N = 5
    H = 67
    W = 103
    test_image_batch = torch.tensor(np.random.uniform(0,1, size=(N,3,H,W)).astype(np.float32))

    tilter = PerspectiveTilter()
    warped, _ = tilter(test_image_batch)
    assert isinstance(warped, torch.Tensor)
    assert warped.shape == test_image_batch.shape